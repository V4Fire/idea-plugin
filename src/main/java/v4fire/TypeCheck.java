...
import v4fire.utils.V4FireConfigFinder;
import v4fire.utils.V4FireExeFinder;
import v4fire.utils.V4FireLinterRunner;
...
public class TypeCheck {
    ...
    static @Nullable TypeDataParams checkCommon(PsiFile file) {
        ...
        if (!V4FireConfiguration.getInstance(project).isEnabled()) {
    ...
    private static String stylusCheck(
        ...
        V4FireState state = getState(project);

        String configPath;

        try {
            assert state.getCustomConfigFilePath() != null;
            configPath = !state.getCustomConfigFilePath().isEmpty() ? state.getCustomConfigFilePath() : V4FireConfigFinder.findPath(project, workingDir);
        } catch (NullPointerException e) {
            return "";
        }

        V4FireLinterRunner.ExtraParams params = new V4FireLinterRunner.ExtraParams();

        params.command = command;
        params.offset = offset;
        params.line = line;

        V4FireLinterRunner.Result result = V4FireLinterRunner.runLint(
               new V4FireLinterRunner.Params(cwd,
